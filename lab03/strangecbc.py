...
class StrangeCBC():
    ...
    def encrypt(self, plaintext: bytes):
        ...
        magic = (1336).to_bytes(16)

        for i in range(0, len(plaintext), self.block_length):
            B = xor(xor(plaintext[i:i + self.block_length], C), magic)
            C = cipher.encrypt(B)
            ciphertext += C
    ...
    def decrypt(self, ciphertext: bytes):
        ...
        ciphertext = self.iv + ciphertext

        for i in range(len(ciphertext), self.block_length, -self.block_length):
            C = ciphertext[i - self.block_length:i]
            C_p = ciphertext[i - 2 * self.block_length : i - self.block_length]
            B = cipher.decrypt(C)
            P = xor(xor(B, C_p), magic)
